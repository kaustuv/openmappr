...
var AlchemyConfig = _.get(AppConfig.get(), 'alchemy');
...
function AlchemyAPI() {
	...
	try {
    var key = AlchemyConfig.ACCESS_KEY; // the real key
	}
	catch(err) {
		//Create the file
    ...
    var postReq = http.request(opts, function (res) {
      ...
      res.on('end', function () {
        try {
          ...
          callback({ status:'ERROR', statusInfo: error.message });
        }
      });
...
AlchemyAPI.ENDPOINTS['combined']['text'] = '/text/TextGetCombinedData'; // Manually added
